...
interface Props {
  ...
  onComment: (sessionId: string, artifactId: string, v: number, comment: string) => void;
}
 ...
 */
export function ArtifactsPane({ sessionId, log, artifacts, onRestore, onComment }: Props) {
  const [tab, setTab] = useState<Tab>("summary");
  const [selected, setSelected] = useState<Record<string, number>>({});
  const [drafts, setDrafts] = useState<Record<string, string>>({});

  const recap = useMemo(() => buildThreadRecap(sessionId, log), [sessionId, log]);

  return (
    <aside className="artifacts-pane" aria-label="Thread summary and artifacts">
      <div className="artifacts-tabs" role="tablist" aria-label="Pane tabs">
        <button
          type="button"
          role="tab"
          aria-selected={tab === "summary"}
          className={tab === "summary" ? "artifacts-tab artifacts-tab-active" : "artifacts-tab"}
      ...
      {tab === "summary" ? (
        <div className="artifacts-body">
          <section>
            ...
            <p className="muted">
              ...
              {recap.counts.assistant} assistant, {recap.counts.subagent} sous-agent,{" "}
              {recap.counts.tool} outil, {recap.counts.system} système.
            </p>
          </section>
          <section>
        ...
        <div className="artifacts-body">
          {artifacts.length === 0 ? (
            <p className="muted">
              Aucun artefact pour l’instant — les blocs de code ou de doc des
              réponses de l’assistant apparaîtront ici en versions.
            </p>
          ) : (
            artifacts.map((a) => {
              const current = selected[a.id] ?? a.versions[a.versions.length - 1]?.v ?? 1;
              const ver = a.versions.find((x) => x.v === current) ?? a.versions[0];
              ...
              return (
                <section key={a.id} className="artifact-card">
                  <header className="artifact-head">
                    <span className="artifact-kind" title={`${a.kind} · ${a.lang !== "" ? a.lang : "plain"}`}>
                      {a.kind === "code" ? (a.lang !== "" ? a.lang : "code") : "doc"}
                    </span>
                    <span className="artifact-title" title={a.title}>
                      {a.title}
                    </span>
                  </header>
                  <div className="artifact-versions" role="group" aria-label={`Versions of ${a.title}`}>
                    {a.versions.map((x) => (
                      <button
                        key={x.v}
                        type="button"
                        aria-pressed={x.v === ver.v}
                        className={x.v === ver.v ? "artifact-ver artifact-ver-active" : "artifact-ver"}
                        onClick={() => setSelected((cur) => ({ ...cur, [a.id]: x.v }))}
                    ...
                    <button
                      ...
                      title="Copier cette version dans le composer"
                    >
                      Restore v{ver.v}
                    </button>
                  </div>
                  ...
                  <label className="artifact-comment-label">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDrafts((cur) => ({ ...cur, [draftKey]: e.target.value }))
                      }
                      onBlur={() => {
                        if (draft !== ver.comment) onComment(sessionId, a.id, ver.v, draft);
                      }}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
