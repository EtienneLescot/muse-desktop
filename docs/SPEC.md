...
- **Posture globale M0-06 (implémentée côté client) :** le réglage **Ask for approval** laisse chaque `approval/requested` à l'utilisateur ; **Approve on my behalf** décide automatiquement les choix dont les scopes restent locaux au workspace et conserve une question pour réseau/élévation ; **YOLO** décide le premier choix non refusé. La posture est persistée sous `muse-desktop.authorization-mode.v1` et ne modifie pas le plafond scellé du host : `session/start` continue d'omettre `approvalMode`, et toutes les décisions passent par `approval/decide` avec le `requirementId` existant.
...
- Prouvé : `stderr_tail` 20 lignes dans les erreurs handshake/host-death ; `evtCount` (diagnostic dev, `useMuseSessions` L257) ; `BUILD_ID="2026-09-08-poll-transport"` (`env.ts` L10, anti-webview stale). Les frames MSP et prompts ne sont pas enregistrés en clair.
