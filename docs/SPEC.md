...
- **M3-09 notifications de runs (partiel) :** `src/lib/notifications.ts` conserve une inbox locale bornée et dédupliquée pour les runs `completed`/`failed`, avec aperçu, session cible, badge non-lu, ouverture et marquage lu dans Automations. La permission `Notification` du webview est demandée uniquement sur action ; en cas de refus ou d'absence d'API, l'inbox reste utilisable. Les toasts ne sont pas rejoués pour l'historique au démarrage. Un service natif OS/Tauri et les notifications de demandes utilisateur restent non implémentés.
