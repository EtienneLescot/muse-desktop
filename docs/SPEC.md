...
| `git_worktree_remove{sessionId,path}` | — (local Git) | exige un chemin canonique déjà présent sous `.muse/worktrees/`, puis retire le checkout après confirmation UI | `main.rs:1192 git_worktree_remove` |
| `worktree_setup_run{sessionId,path,command}` | — (local process) | exécute uniquement une commande saisie explicitement dans un worktree existant sous `.muse/worktrees/` ; stdin désactivé, commande ≤2 000 caractères, sortie ≤200 000 caractères, timeout 10 min ; renvoie `ready|failed|timedOut`, sortie, code et durée | `main.rs:1205 worktree_setup_run` |
