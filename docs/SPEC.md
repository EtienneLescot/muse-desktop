...
- **US-9 (C-V2+) :** automations = instructions+skills planifiées background, résultats consultables et revue humaine selon la politique [ref-codex] ; scheduled récurrentes/one-shot (ex. lun/mer 9h), threads réutilisés [ref-claude/antigravity]. **AC :** création schedule, déclenchement à l'heure, contexte workspace/projet/modèle/politique capturé ; `ask` passe par la review queue, `workspace`/`yolo` dispatchent sans clic et journalisent l'occurrence ; rattrapage `latest/skip`, idempotence et retries bornés. **Limite actuelle :** le timer reste dans l'app et `completed` signifie `send_input` acquitté, faute d'événement MSP de fin de turn exploitable. [TROU : `workflow/*` sans ligne schéma (spec 14410 open) → scheduler natif et parallélisme workflow non spécifiables sur MSP.]
...
- Surface IPC (`main()` `invoke_handler!`) : commandes `start_session / set_approval_mode / restore_sessions / send_input / unqueue_turn / approve / answer_input / cancel_input / cancel_session / kill_session / set_workspace / read_session_history / list_pending_requests / poll_events`. Événements front : `output, subagent_event, tool_request, status` (+ `input_request`, `input_settled` via `emit()` L107).
...
- `persist.ts` : clés `muse-desktop.{sessions,workspace,active,tombstones}.v1` + `muse-desktop.log.v1.{sessionId}` (L36-40), avec la façade défensive `storage.ts`. Les autres registres locaux (automations, notifications, index, mémoire, artifacts, imports, partage, skills, navigateur, composer, thème et réglages de session) utilisent également cette façade ; les clés et formats `v1` ne changent pas. `StoredSession{session_id, workspace, title, createdAt}`, `LogEntry{id, ts, role:user|assistant|thinking|subagent|system|tool, text, agentId?, itemId?, open?}`. `MAX_LOG_ENTRIES=2000` (append-only, `slice(-2000)` sur `appendLog`/`saveLog`), `MAX_TOMBSTONES=500` (`saveTombstones(ids.slice(-500))`). `loadLog` ferme les blocs `open` restés ouverts (nouveaux chunks → nouvelles entrées). Les lectures invalides reviennent au fallback sans exception et enregistrent un diagnostic borné (`unavailable`, `corrupt`, `quota`) ; les écritures conservent la valeur précédente en cas d'échec. `SettingsPanel` propose l'export et l'import local d'un snapshot JSON : les entrées sont bornées au namespace `muse-desktop.*`, les clés existantes sont conservées par défaut et le remplacement exige une confirmation, puis le boot normal réhydrate les sessions. Aucun appel réseau ni moteur n'est déclenché par l'import. Les migrations de versions antérieures et la restauration sélective restent **[TROU]**.
- `useMuseSessions` : boot restaure sessions/logs/workspace/activeId (sessions marquées `running:false` — les enfants ne survivent pas au restart), filtre tombstones ; write-through (`saveSessions` sans `running`, `saveActiveId`, `saveWorkspace` sauf `null`) ; `killSession` → tombstone + `dropLog` + purge approvals/inputs + réassignation activeId. Après `resume_session`, `read_session_history` demande un `session/read` inline et `history.ts` fusionne les items pliés par `itemId` (ou texte utilisateur pour les anciens échos locaux), conserve les notes locales et remplace les blocs streamés partiels par leur état durable.
...
| `worktree_setup_run{sessionId,path,command,operationId}` | — (local process) | exécute uniquement une commande saisie explicitement dans un worktree existant sous `.muse/worktrees/` ; stdin désactivé, commande ≤2 000 caractères, sortie ≤200 000 caractères, timeout 10 min ; renvoie `ready|failed|timedOut|cancelled`, sortie, code et durée | `main.rs:1218 worktree_setup_run` |
| `worktree_setup_readiness{sessionId,path}` | — (local filesystem/PATH) | inspecte sans exécuter de code les manifests `package.json`, `Cargo.toml`, `pyproject.toml`, `go.mod` et les exécutables requis ; renvoie `ready|blocked|needsSetup`, les outils détectés et l'horodatage | `main.rs:1250 worktree_setup_readiness` |
| `worktree_setup_cancel{sessionId,operationId}` | — (local process) | demande l'arrêt du setup ciblé via son identifiant d'opération ; renvoie `true` si le worker était encore enregistré. Le résultat terminal `cancelled` est produit par le worker après arrêt du processus | `main.rs:1250 worktree_setup_cancel` |
...
| `skills_read_resources{workspace?,skillPath,resourcePaths}` | — (local filesystem) | relit au dernier moment jusqu'à 20 ressources UTF-8 dans le dossier du `SKILL.md`, avec contenu borné, chemins et erreurs ; les traversées, liens et fichiers binaires sont refusés | `main.rs:skills_read_resources` |
...
| `read_session_history{sessionId}` | `session/read` | `{sessionId,excludeItems:false}` ; lecture ponctuelle sans lease ni réémission de demandes ; renvoie l'enveloppe `history` (items inline ou snapshot) après validation d'identité et de workspace | `main.rs:1789 read_session_history` |
| `list_pending_requests{sessionId}` | `approval/listPending` | `{sessionId}` ; lecture ponctuelle des approvals et questions suspendues, sans lease ni réémission ; les tokens de requirement restent ceux fournis par le host | `main.rs:1815 list_pending_requests` |
...
| `unqueue_turn{sessionId,turnId}` | `turn/unqueue` | `{commandId:UUIDv7,sessionId,turnId}` ; reclaim explicite d'un tour encore en queue ; ne se transforme jamais en interruption après lancement | `main.rs:2000 unqueue_turn` |
