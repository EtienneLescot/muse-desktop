...
| **M1-01→04** | git review end-to-end on a scratch repo (`m1-01-04-git-native.json`, `m1-04-pr-roundtrip.json`) | **Windows closed** — snapshot lists the files changed during a controlled turn (`config.properties` + `notes.txt`); the office file shows git's binary marker in the diff view; an 802-row status renders with the true count; stage/unstage by file (`M ` / ` M` verified in porcelain), **partial hunk staging** (1 hunk staged + 1 left unstaged via Unstage hunk), discard reverts content after the inline confirm, **untracked files survive** every discard path; a diff-line comment reaches the **live engine** (turn starts, answer streams) and a queued comment persists (`ready` + `sent` items); commit lands (`qualif: gamma clarifier change`), push writes `refs/heads/master` on the bare remote, Fetch reports the advance and **Pull latest fast-forwards HEAD** (`60406fc4 → 8a15aaa8`); a forbidden remote surfaces a clean actionable error (`Repository not found`). **The live PR round trip is closed too** (`m1-04-pr-roundtrip.json`): the Ship panel pushed `qualif-pr` to a disposable private GitHub repo, opened [m1-qualif-pr#1](https://github.com/EtienneLescot/m1-qualif-pr/pull/1) through the real form, `gh` confirmed it independently, and a second click reported **"Existing pull request"** (idempotent); the PR was closed after the run and the repo awaits manual deletion (the gh token has no `delete_repo` scope). |
...
- M1-09 host fork refusal (above) — reported upstream as [muse-code-sdk#55](https://github.com/meta-models/muse-code-sdk/issues/55).
- The disposable qualification repo [EtienneLescot/m1-qualif-pr](https://github.com/EtienneLescot/m1-qualif-pr) awaits manual deletion (the gh token has no `delete_repo` scope); its PR was closed.
