...
- Visual capture is available only as an explicit `getDisplayMedia` gesture in
  ...
  shows a preview and keeps URL/time/viewport/DPR next to the image before it
  is inserted into the composer. The runtime cannot force the chooser to pick
  the Muse Browser window, so a capture is never treated as proof that the
  selected pixels came from the URL without that user check.
...
safe image payloads and the attachment size bound. The browser panel keeps a
