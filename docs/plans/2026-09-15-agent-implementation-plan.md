...
**Reste :** l'injection couvre maintenant la boucle `CommandEvent` réelle, y compris les chunks stdout partiels, les fragments finaux sans saut de ligne, les erreurs du shell et la fermeture du receiver qui réveille le client. Il reste à qualifier l'appel `invoke` de la webview et le scénario A/B natif avec approbations. Ces tests restent séparés d'un tour modèle réel.
