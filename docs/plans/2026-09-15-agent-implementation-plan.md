...
**Fondation d'injection livrée :** le transport MSP dépend maintenant d'une interface enfant minimale, avec adaptateur `CommandChild` en production. Les tests Rust peuvent donc conduire la corrélation de requêtes, l'isolation A/B et le réveil des appels en attente avec un enfant déterministe sans démarrer de moteur modèle. Cette frontière prépare le pilote de fixture du superviseur ; elle ne constitue pas encore le scénario Tauri complet.
