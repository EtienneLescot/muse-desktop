...
**Tranche livrée :** le snapshot exporté peut être réimporté depuis Settings. L'import vérifie le format/version, limite les clés au namespace `muse-desktop.*`, restaure les clés absentes sans écraser l'existant, puis permet un second passage confirmé pour remplacer les clés. Le rechargement suit le boot normal, ce qui évite un état React partiellement restauré.
