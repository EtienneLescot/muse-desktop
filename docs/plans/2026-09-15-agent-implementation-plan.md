...
**État au 18/09/2026 :** demande de cancellation conservée dans un état renderer-only jusqu'à réception d'un terminal `turn/completed`, `turn/retracted` ou `turn/stopped` (ou déconnexion), avec badge `Stopping Muse`, bouton désactivé contre le double-clic et transcript non fermé prématurément. Le bridge transmet désormais le `turnId` observé quand il est disponible, tout en gardant le payload sans cible pour les hosts anciens. Si l'accusé reste sans événement pendant la fenêtre de liveness, le fil affiche un état de récupération et une fermeture explicite au lieu de mouliner indéfiniment. La qualification native des courses et réponses tardives reste à produire.
...
**État au 19/09/2026 :** les cartes d'approbation et de questions placent le focus à l'arrivée, proposent la navigation fléchée des choix, une boucle Tab confinée à la carte active, ainsi que Ctrl+Entrée pour répondre et Échap pour ignorer. Le transcript reçoit désormais le focus et expose Home/End/PageUp/PageDown avec des cibles bornées ; End réutilise le retour au dernier message et le chargement des pages anciennes reste déclenché par le scroll. La feuille de style ajoute aussi un mode `prefers-contrast: more` qui renforce focus, conversation active et messages secondaires sans changer la palette par défaut. Les tests purs de navigation et d'annonces restent verts ; la vérification avec lecteur d'écran réel, zoom 200 %, contraste natif et réduction des mouvements demeure à exécuter.
...
**Tranche livrée (19/09/2026) :** le renderer capture l'observation `git_status` juste avant chaque nouveau tour admis et la conserve dans la SSOT par conversation avec son `clientMessageId`, `turnId` lorsqu'il est fourni et état d'admission. Le baseline est persisté sous un namespace dédié, restauré au redémarrage et supprimé avec la conversation. La sonde est bornée à 1,5 seconde afin qu'un Git bloqué ne puisse pas faire attendre le composer. Le panneau Review compare ensuite cette observation avec le statut courant et expose les chemins dont l'état a réellement changé, ainsi qu'un changement de HEAD ou de fingerprint sans fabriquer de patch. Un échec de capture ne bloque jamais un envoi dans un workspace non-Git ; l'absence de baseline reste visible par l'absence de la carte, jamais comme un faux résultat.
...
**Reste :** le protocole MSP ne fournit pas encore d’annulation atomique confirmée ni de résultat métier complet garanti. Un lease renderer et un verrou OS advisory Tauri bornés au workspace protègent maintenant les panneaux et processus Muse concurrents ; le descripteur natif reste ouvert pendant le lease, ce qui permet la libération automatique du verrou après un crash, et l’UI le libère sur fin, arrêt, échec ou changement de workspace. Un accusé de Stop conserve le lease et l’état `stopping` jusqu’à l’observation terminale `writerSessionRunning=false`, et la commande porte le `turnId` actif quand le host l’a annoncé. Le panneau calcule désormais, à la fin d’une session inactive, un résumé extractif local du journal (compteurs, dernière sortie assistant et erreurs structurées). Lorsqu’un événement terminal host fournit `resultPreview` ou une erreur structurée, ce résultat est propagé par la SSOT de session et affiché en priorité ; les listes structurées d’issues et prochaines étapes sont également affichées dans le résultat Writer ; l’observation locale reste le repli lorsque le host ne fournit rien. Garder le dispatch sur le chemin normal start/send tant que le contrat MSP n’est pas vérifié ; ajouter ensuite l’annulation atomique confirmée et la collecte de résultat métier native avec une intégration séparée des changements.
...
**État au 19/09/2026 :** première passe UI livrée dans `BrowserPanel` : URL normalisée, historique précédent/suivant, Reload, URL effectivement affichée, erreurs de chargement/protocole et jusqu'à huit onglets locaux restaurés sous `muse-desktop.browser.tabs.v1`. La navigation est maintenant persistée sous une clé dérivée du `sessionId` de conversation et le panneau est remonté lors d'un changement de conversation ; une session ne peut donc plus récupérer les onglets d'une autre. La persistance ne contient que des URL http(s) et un historique borné. Un téléchargement explicite de lien same-origin est maintenant disponible : en desktop, `browser_download_fetch` fait la requête native sans credentials, refuse les redirections, borne le flux à 10 MiB et renvoie un payload base64 au dialogue de destination Tauri ; le preview web conserve un fetch sans credentials puis un Blob borné. Les liens `<a download>` déclenchés par une page same-origin sont interceptés et réutilisent ce même parcours borné, avec le grant computer-use **browser**. Le nom de fichier est réduit à un basename sûr et l'écriture desktop est validée côté Rust. **Open native** dérive désormais une fenêtre dédiée du `sessionId`, ferme une surface d'une autre conversation avant ouverture et active le profil privé non persistant ; **Close native** vise uniquement la conversation courante et reste idempotent. La sandbox iframe reste une prévisualisation bornée ; les réponses de téléchargement initiées par la navigation et la qualification multi-plateforme restent à qualifier.
...
**État au 19/09/2026 :** l'ancre textuelle utilise l'URL normalisée réellement affichée et les notes sont filtrées par cette URL. Dans une iframe same-origin, un clic produit une ancre DOM bornée (sélecteur, balise, rôle, libellé et texte) qui reste de la métadonnée passive. **Add page context** et **Add to prompt** insèrent dans le composer actif un bloc borné avec provenance, URL, sélection, commentaire et ancre élément. **Capture visible page** demande explicitement une source via `getDisplayMedia`, affiche un aperçu avec un contrôle de zoom de revue borné à 100–250 % et un viewport défilable, permet de sélectionner et recadrer une région, ajoute l'image capturée comme pièce jointe MSP et conserve URL, heure, viewport, DPR, coordonnées de région et ancre élément dans le contexte ; le hook de sessions centralise le pré-remplissage et l'attachement. La demande de capture conserve la génération de navigation et l'URL normalisée ; si l'utilisateur navigue ou recharge pendant le sélecteur d'écran, les pixels périmés sont refusés avant d'atteindre le composer et le panneau demande une nouvelle capture. Le dimensionnement explicite de l'image et du canvas maintient la correspondance entre le pointeur et les pixels source, y compris lorsque l'aperçu est zoomé. Les pages cross-origin conservent le fallback manuel ; la capture automatique de la seule iframe et la qualification native restent à produire.
...
**État au 19/09/2026 :** l'export local Markdown/JSON est maintenant sauvegardable via le dialogue natif dans le desktop, avec repli téléchargement dans le preview web et erreur visible en cas d'échec. Avant création, les valeurs credential-shaped connues sont remplacées, les entrées sont plafonnées à 400 et 12 000 caractères par entrée, le corps à 240 000 caractères, et le bundle persiste `redacted`/`truncated`/`omittedEntries` pour que l'interface signale une exportation bornée. La SSOT conserve au plus 100 bundles, y compris l'historique révoqué, afin de rester récupérable après de nombreux exports. Le bundle reste explicitement local et révocable uniquement dans le profil ; aucune URL publique n'est fabriquée. Reste le service d'hébergement, l'identité, la durée de vie et la révocation observables depuis un second client.
...
Ne déclarer un parent terminé que lorsque tous ses critères d'acceptation sont couverts. Le prochain lot conseillé est la **preuve native M0-01c/M0-02c** (deux workspaces, panne/reconnexion, approbation et reprise d'un tour), maintenant que les chemins renderer et liveness sont câblés. M2-08 dispose désormais du dispatch renderer vers les conversations de worktree, d'un verrou natif par cible et de la projection des complétions host disponibles ; l'annulation atomique et le résultat métier complet restent conditionnés au contrat writer/spawn du moteur. M3-07 dispose d'un réveil ponctuel Windows, macOS et Linux ; la qualification OS, la reprise après sommeil/verrouillage et la preuve host restent ouvertes.
