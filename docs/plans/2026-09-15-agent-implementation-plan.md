...
**État :** héritage global → projet visible via le helper SSOT `settingsForThread`, avec diff d’override et contexte de la conversation. Le modèle effectif est appliqué à la nouvelle session après `session/start` via `session/setModel`; la valeur `default` conserve le choix du moteur. Le compactage automatique local respecte maintenant la valeur `autoCompact` effective par conversation. La préférence d'isolation globale est maintenant traduite au lancement du host selon le contrat `muse serve --help` (`workspace` réseau restreint, `network`, `elevated`), tandis qu'un host déjà vivant conserve sa posture fixe. L'en-tête et Settings rendent cette limite explicite ; les overrides sandbox/réseau de projet restent en attente d'un mapping host séparé.

**Reste :** mapper explicitement les overrides sandbox/réseau propres aux projets vers les flags host sans laisser une préférence globale silencieuse les écraser ; aucun host ne permet encore de muter cette posture pendant sa durée de vie. Ajouter ensuite la qualification native de deux projets utilisant des hosts distincts et le redémarrage contrôlé d'un host dont la posture demandée change.
