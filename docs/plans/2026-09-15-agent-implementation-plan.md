...
**Reste :** le protocole MSP ne fournit pas encore de résultat writer structuré ni d’annulation atomique confirmée. Un lease renderer et un verrou OS advisory Tauri bornés au workspace protègent maintenant les panneaux et processus Muse concurrents ; le descripteur natif reste ouvert pendant le lease, ce qui permet la libération automatique du verrou après un crash, et l'UI le libère sur fin, arrêt, échec ou changement de workspace. Un accusé de Stop conserve le lease et l'état `stopping` jusqu'à l'observation terminale `writerSessionRunning=false`. Le panneau calcule désormais, à la fin d'une session inactive, un résumé extractif local du journal (compteurs, dernière sortie assistant et erreurs structurées) ; il est étiqueté comme observation et ne remplace pas la collecte native. Garder le dispatch sur le chemin normal start/send tant que le contrat MSP n’est pas vérifié ; ajouter ensuite l'annulation atomique et la collecte de résultat native avec une intégration séparée des changements.
