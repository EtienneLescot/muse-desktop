...
**Sous-ticket liveness (livré) :** définir un pur `classifyStreamHealth` avec seuil borné (15 s), priorité aux demandes d'action et états `waiting-host`/`stalled`. Toucher l'activité sur chaque événement et sur les actions qui relancent le tour (`send`, approbation, question, guidance). Afficher le signal inline dans `StreamView`, conserver les détails de réflexion repliables et vérifier la frontière du seuil, les états explicites et le rendu sans backend. La qualification d'un vrai host qui reste silencieux doit rester une preuve native séparée.
