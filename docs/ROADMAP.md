...
| M0-02 | Reprendre une conversation après fermeture ou panne du moteur | Adapté | Présente | Partielle | UI | Reconnexion explicite câblée via session/read + session/resume ; réhydratation des items pliés côté client par `itemId` avec conservation des notes locales ; tests moteur sans tour et UI avec IPC simulé réussis. Reste : demandes en attente et reprise d'un tour via UI native |
...
- **Limites :** les variables d'environnement autorisées, la création atomique session → worktree et le signal de readiness globale restent à concevoir pour M2-05/M2-06. Le setup est donc une action explicite et locale.
...
| M2-04 | Préparer l'environnement du worktree | Adapté | Présente | Partielle | Intégration | Commande explicite, profils persistants par workspace, annulation native ciblée, états et sortie bornée livrés ; restent variables autorisées et readiness globale |
