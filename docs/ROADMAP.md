...
- **M0-14g — pump stdout partiellement prouvé** : `pump_stdout` délègue désormais à `ingest_stdout_chunk`, qui est testé avec une fixture enfant injectée : frames MSP partielles et multiples, notifications et ligne illisible sont traitées par la même frontière que la production, avec diagnostic borné. L'appel `invoke` depuis une webview empaquetée, le receiver `CommandEvent` réel et le scénario natif A/B complet avec approbations restent à ajouter avant de déclarer M0-14 complet.
- **Validation** : `npm test` (564 tests Node, dont cinq tests fixture), build frontend et 112 tests Rust du superviseur verts. La fixture s'exécute aussi dans le job Node de la CI depuis un clone propre.
