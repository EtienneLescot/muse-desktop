...
| M1-08 | Ajouter fichiers et images à une demande | Adapté | Présente | Câblée | Intégration | Texte borné et images base64 sont envoyés comme parts MSP réelles ; dimensions détectées et aperçu miniature livrés quand le runtime navigateur le permet ; restent validation live sur les modèles image et reprise d’un fichier disparu |
...
| M1-10 | Réorienter une exécution ou mettre un message en attente | Adapté | Présente | Câblée | Unitaire | Queue MSP par défaut et disposition `queued`/`steered` visibles ; les tours admis en queue restent listés, leur ordre est persisté et peut être retiré avant lancement via `turn/unqueue`. Après relance sans snapshot host, les rappels sont marqués à vérifier et ne sont jamais rejoués automatiquement ; qualification live restante |
...
| M1-13 | Lire une longue conversation confortablement | Adapté | Présente | Partielle | UI | Fenêtre DOM bornée au-delà de 600 entrées (160 visibles, chargement de 120 anciens), scroll compensé et restauration de position par conversation livrés ; restent mesure native à 2 000 entrées, recherche native hors fenêtre et qualification assistive |
