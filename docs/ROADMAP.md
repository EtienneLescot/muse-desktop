...
- **M0-14c — fixture livrée** : `scripts/msp-fixture.mjs` fournit les scénarios `success`, `interleaved`, `reject` et `drop`. `test/msp-fixture.test.ts` lance un vrai processus enfant, vérifie le framing `Content-Length`, les notifications `tools/list_changed`, les erreurs JSON-RPC et la fermeture stdout sans toucher à un workspace.
...
- **Limites :** le serveur n'est lancé qu'après une action explicite et s'arrête à la fermeture de l'app ou via **Stop server** ; les outils restent un catalogue local tant que le host Muse ne fournit pas de bridge MCP. Mise à jour de package/source, injection moteur et permissions d'appel restent à traiter.
