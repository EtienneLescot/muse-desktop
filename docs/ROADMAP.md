...
- **M0-01b — smoke moteur reproductible** : `npm run smoke:native` lance deux `muse serve --no-session-log` réels sur Windows, valide `initialize/initialized`, `session/start` dans deux dossiers temporaires, l'unicité des sessions et `model/list` sur chaque hôte, puis tue les deux processus et supprime les dossiers. La preuve du 17 septembre 2026 a renvoyé deux sessions distinctes avec le sidecar Muse Code 1.3.0 ; aucun tour modèle n'est envoyé par défaut. Le contrôle explicitement opt-in `node scripts/native-smoke.mjs --exercise-control` admet en parallèle puis interrompt un tour par hôte et vérifie le même `turnId` dans chaque accusé d'interruption. Ce harnais exerce les moteurs réels ; les tests Rust exercent le registre du superviseur.
