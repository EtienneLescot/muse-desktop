...
- **M0-14g — pump stdout partiellement prouvé** : `pump_stdout` délègue désormais à `ingest_stdout_chunk`, qui est testé avec une fixture enfant injectée : frames MSP partielles et multiples, notifications, lignes illisibles, erreurs du shell, fragments finaux sans saut de ligne et fermeture inattendue du receiver sont traités par la même frontière que la production, avec diagnostic borné et session réveillée sur perte du canal. Les handlers Tauri `send_input` et `approve` sont aussi exercés via l’invoke mocké avec un état de session réel, y compris deux sessions partageant un identifiant d’approbation ; l'appel depuis une webview empaquetée et le scénario natif A/B complet avec approbations restent à ajouter avant de déclarer M0-14 complet.
- **Validation** : `npm test` (564 tests Node, dont cinq tests fixture), build frontend et 117 tests Rust du superviseur verts. La fixture s'exécute aussi dans le job Node de la CI depuis un clone propre.
