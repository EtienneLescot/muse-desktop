...
- **Limites :** le serveur n'est lancé qu'après une action explicite et s'arrête à la fermeture de l'app ou via **Stop server** ; les outils restent un catalogue local tant que le host Muse ne fournit pas de bridge MCP. Mise à jour de package, rollback, injection moteur et permissions d'appel restent à traiter.
...
| M3-03 | Installer/désactiver une extension réellement utilisable | Adapté | Présente | Partielle | Intégration | Enregistrement post-probe, runtime persistant explicite, hot-list et hot-reload du registre livrés ; restent package/update/rollback et injection dans le moteur |
