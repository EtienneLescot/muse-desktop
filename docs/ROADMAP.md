...
| M1-04 | Synchroniser, commit, push et création de PR depuis l'app | Adapté | Présente | Câblée | Intégration | Fetch explicite et pull fast-forward-only ajoutés avec garde de statut ; restent qualification hooks/auth live, PR existante/rejet distant et revue native |
...
- **Validation :** tests Rust sur dépôt temporaire pour commit, garde d'empreinte et remote absent, puis fetch local et pull fast-forward ; tests de sécurité sur refs ; suite complète **599 Node**, build TypeScript/Vite et **135 Rust** tests passent.
