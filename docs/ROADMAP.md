...
| ID | Résultat attendu | Design | UI | Fonction | Validation | Reste à faire et critère de sortie |
|---|---|---|---|---|---|---|
| M4-01 | Naviguer dans un vrai navigateur intégré | Adapté | Présente | Partielle | Unitaire | Navigation URL normalisée, historique précédent/suivant, onglets locaux bornés, rechargement et erreurs d'iframe livrés ; action **Open native** ouvre une webview Tauri dédiée avec validation http(s) et réutilisation de fenêtre ; cookies, téléchargements et sessions maîtrisées restent à qualifier |
