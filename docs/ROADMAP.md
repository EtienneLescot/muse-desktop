...
| M4-02 | Annoter visuellement une page et transmettre le contexte | Adapté | Présente | Partielle | Unitaire | Ancre URL normalisée, sélection/commentaire, capture visuelle explicite et recadrage d'une région sont livrés ; capture automatique iframe/zoom et qualification native restent à concevoir |
...
- **Contexte vers la conversation :** les actions **Add page context** et **Add to prompt** transmettent au composer actif un bloc borné et explicitement étiqueté avec l'URL normalisée, la sélection et le commentaire. Le bouton **Capture visible page** demande une source d'écran via `getDisplayMedia`, affiche un aperçu, permet de dessiner puis recadrer une région, conserve l'URL/heure/viewport/DPR et la région d'origine, puis insère l'image comme vraie pièce jointe avec son bloc de provenance. Le hook de sessions reste la SSOT et refuse les captures invalides, une session absente ou un contexte vide.
- **Limites :** l'iframe sandboxée reste le repli du web preview et ne fournit ni cookies/onglets d'un navigateur complet. La capture dépend de l'API de partage d'écran du runtime et reste un geste explicite : l'utilisateur doit choisir la surface correspondant à l'URL affichée. Capture automatique de la seule iframe, ancre d'élément DOM, iframe/zoom, sessions/cookies, téléchargements et qualification WebView2 restent à qualifier. Décision : `docs/plans/2026-09-17-native-browser-surface.md`.
