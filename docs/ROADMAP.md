...
État de référence : **17 septembre 2026**, après fusion de la chaîne de PRs #18–#95 et correction des remarques CodeRabbit actionnables. Ordre M0 → M4 validé par Étienne. Objectif : finir les parcours existants, puis atteindre la parité des workflows desktop de Codex en conservant le branding Muse.
...
- **Chaîne livrée :** les PRs #18 à #95 ont été fusionnées dans l'ordre de leurs branches de base. La branche finale contient les parcours M0 à M4 déjà câblés dans le client, ainsi que les correctifs de reprise, transcript, terminal Muse, observations Git, file de commentaires de revue persistante, résumés extractifs de runs, l'injection MCP opt-in des sessions locales/distantes, le cycle de vie des bundles MCPB, le secret-store natif et la révocation explicite des credentials distants, ainsi que le fallback de démarrage natif.
