...
| M0-02 | Reprendre une conversation après fermeture ou panne du moteur | Adapté | Présente | Partielle | UI | Reconnexion explicite câblée via session/read + session/resume ; réhydratation des items pliés côté client par `itemId` avec conservation des notes locales ; liveness du host visible dans le stream avec attente d'action et état stale ; tests moteur sans tour et UI avec IPC simulé réussis. Reste : validation native des demandes en attente et reprise d'un tour |
