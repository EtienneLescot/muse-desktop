...
Validation du socle audité : build frontend, 573 tests Node, 125 tests Rust verts. La dernière passe ajoute le repli des items MSP complets sans delta et applique les remplacements de snapshot pendant les `item/updated` encore `inProgress`, sans fermer ni concaténer la lane. Les preuves UI de la passe précédente sont détaillées dans [la passe conversations](plans/2026-09-14-conversation-polish.md). Elles ne couvrent pas l'ensemble des critères futurs ci-dessus.
