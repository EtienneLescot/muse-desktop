...
État de référence : **17 septembre 2026**, après fusion de la chaîne de PRs #18–#98 et correction des remarques CodeRabbit actionnables. Ordre M0 → M4 validé par Étienne. Objectif : finir les parcours existants, puis atteindre la parité des workflows desktop de Codex en conservant le branding Muse.
...
- **Preuves reproductibles :** `npm test` — **599 tests passés** ; `npm run build` — **vert** ; `cargo test --manifest-path src-tauri/Cargo.toml` — **135 tests passés**. Le smoke Windows complet (deux sidecars réels) couvre désormais aussi la lecture de reprise et la liste des demandes en attente, en signalant explicitement les méthodes absentes du host ; les builds Windows NSIS/MSI restent des preuves séparées, documentées dans les livraisons M0/M4.
...
| M0-14 | Disposer de contrôles reproductibles avant fusion | — | — | Partielle | Unitaire | CI build/Node/Rust et artefacts d'échec bornés ajoutés sans credentials ni binaire sidecar ; fixture MSP et pilote de `send_input` injecté livrés ; la perte du ring d'événements est détectable côté polling ; restent le scénario A/B depuis l'interface Tauri et la traversée d'une vraie fixture enfant par le pump stdout |
...
- **M0-01b — smoke moteur reproductible** : `npm run smoke:native` lance deux `muse serve --no-session-log` réels sur Windows, valide `initialize/initialized`, expose `sessionDurability` dans le rapport, crée une session dans deux dossiers temporaires, vérifie l'unicité des sessions et `model/list` sur chaque hôte, puis termine l'arbre de processus Windows et supprime les dossiers. La preuve du 17 septembre 2026 a renvoyé deux sessions distinctes avec le sidecar Muse Code 1.3.0, annoncé `ephemeral` ; aucun tour modèle n'est envoyé par défaut. Le contrôle combiné `node scripts/native-smoke.mjs --exercise-control --exercise-errors --exercise-approval --exercise-isolation` a aussi réussi : deux interruptions corrélées, quatre erreurs structurées (`methodNotFound`/`invalidParams`), six tentatives de posture avec le plafond `approval_mode_ceiling` observé et B arrêté alors que A reste joignable. Ce harnais exerce les moteurs réels ; les tests Rust exercent le registre du superviseur.
- **M0-01b — smoke moteur reproductible** : `npm run smoke:native` lance deux `muse serve --no-session-log` réels sur Windows, valide `initialize/initialized`, expose `sessionDurability` dans le rapport, crée une session dans deux dossiers temporaires, vérifie l'unicité des sessions et `model/list` sur chaque hôte, puis termine l'arbre de processus Windows et supprime les dossiers. La preuve du 17 septembre 2026 a renvoyé deux sessions distinctes avec le sidecar Muse Code 1.3.0, annoncé `ephemeral` ; aucun tour modèle n'est envoyé par défaut. Le contrôle combiné `node scripts/native-smoke.mjs --exercise-control --exercise-errors --exercise-approval --exercise-isolation --exercise-user-shell --exercise-reconnect` a aussi réussi : deux interruptions corrélées, quatre erreurs structurées (`methodNotFound`/`invalidParams`), six tentatives de posture avec le plafond `approval_mode_ceiling` observé, deux sondes `userShell` admises et B arrêté alors que A reste joignable. La sonde de réconciliation a interrogé `session/read` et `approval/listPending` sur chaque host ; le sidecar 1.3.0 répond `methodNotFound`, donc le rapport les marque `unsupported` au lieu de transformer une absence de contrat en faux succès. Ce harnais exerce les moteurs réels ; les tests Rust exercent le registre du superviseur.
...
- **M0-14g — pump stdout partiellement prouvé** : `pump_stdout` délègue désormais à `ingest_stdout_chunk`, qui est testé avec une fixture enfant injectée : frames MSP partielles et multiples, notifications, lignes illisibles, erreurs du shell, fragments finaux sans saut de ligne et fermeture inattendue du receiver sont traités par la même frontière que la production, avec diagnostic borné et session réveillée sur perte du canal. Les handlers Tauri `send_input`, `approve` et `answer_input` sont aussi exercés via l’invoke mocké avec un état de session réel, y compris deux sessions partageant un identifiant d’approbation ; l'appel depuis une webview empaquetée et le scénario natif A/B complet avec approbations restent à ajouter avant de déclarer M0-14 complet.
- **Validation** : `npm test` (564 tests Node, dont cinq tests fixture), build frontend et 118 tests Rust du superviseur verts. La fixture s'exécute aussi dans le job Node de la CI depuis un clone propre.
...
- **M0-02d — corrigé côté client** : après une décision d'autorisation, la chaîne de polling est relancée immédiatement et les nouveaux items ne restent plus bloqués sur l'indicateur réflexif. Une résolution reçue après reconnexion restaure aussi le pont `Muse is resuming` et son placeholder sans exiger que la promesse du clic soit encore en mémoire. Les approbations composées conservent la carte quand le host renvoie `terminal:false`, appliquent le nouveau `currentRequirementId` et remplacent les choix sur `approval/updated`. Le clic est maintenant classifié depuis le snapshot d'approbation courant et borné à `(sessionId, approvalId, choiceId)`, afin qu'un refus connu ne puisse pas afficher un faux état de reprise ; les items de raisonnement sont séparés du texte de réponse et peuvent être dépliés ; il reste à valider ce flux avec un moteur live qui émet un item `reasoning`.
...
- **Bornage DOM :** chaque page remplace la fenêtre visible au lieu de s’y ajouter ; même après plusieurs chargements d’anciens messages, le transcript conserve au plus 160 nœuds rendus. L’avis de fenêtre indique la plage réellement affichée.
...
- **Observabilité :** le journal porte `data-entry-count` afin de mesurer en UI native la taille complète de session, et `aria-label="Conversation messages"` garde une cible stable pour les essais assistifs. Chaque entrée montée expose désormais un rôle `article`, sa position globale (`aria-posinset`) et la taille complète (`aria-setsize`), même lorsque la fenêtre DOM ne contient qu'une page ; un statut live annonce la plage affichée après un chargement ou un saut. Les helpers de fenêtre et de métadonnées assistives sont purs et couverts par tests.
...
- **Validation :** le cycle clavier du finder et les métadonnées de positionnement/annonces sont couverts par des helpers purs ; TypeScript, build Vite et suite Node complets restent verts. La mesure native à 2 000 entrées et la qualification lecteur d’écran réelle sont encore à produire.
...
| M3-06 | Exécuter un travail planifié sans clic préalable | Adapté | Présente | Partielle | Intégration | Chaque schedule/review capture workspace, projet, modèle et politique ; la cible vérifie maintenant aussi son rattachement projet et réutilise les réglages projet effectifs ; ask reste en revue, workspace/YOLO dispatchent automatiquement ; journal local borné des runs visible ; une fin de tour structurée marque maintenant le run completed/failed et applique le retry borné si le host le permet. Le résumé extractif local est capturé avec le résultat ; restent le scheduler natif hors cycle UI et un signal métier fourni directement par le host |
| M3-07 | Gérer sommeil, reprise, doublons et échecs de planning | Adapté | Présente | Partielle | Intégration | Politique skip/latest, curseur d'occurrence stable, bail inter-fenêtres, claim anti-doublon, retries bornés avec backoff/annulation, réveil immédiat au retour de fenêtre et fuseau IANA capturé avec résolution DST livrés côté client ; après redémarrage, les runs non terminaux sont marqués **Review needed** et bloqués jusqu'à une réconciliation explicite pour éviter les doublons ; scheduler natif multi-instance et reprise après crash côté host restent ouverts |
| M3-08 | Examiner les résultats des runs | Adapté | Présente | Partielle | Intégration | Historique borné, aperçu, statut, non-lu, lien vers la conversation, archivage, filtres, retry manuel et inspecteur de contexte livrés ; résumé extractif borné (headline, compteurs, fichiers, issues) affiché et persisté ; résumé métier sémantique et fin de run native restent ouverts |
...
- **Routage direct :** les notifications Tauri déclarent une action **Open conversation** et transportent uniquement `notificationId`, `sessionId` et `runId` dans `extra`. `App` valide le `sessionId` contre les sessions connues avant d'ouvrir le fil ; le fallback Web émet le même événement borné. Un identifiant absent ou inconnu n'effectue aucune navigation.
