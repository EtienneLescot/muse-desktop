...
| M0-07 | Protéger les diagnostics et éviter un crash sur Unicode | Adapté | Présente | Partielle | Unitaire | Aucun wire log brut par défaut ; stderr borné à 20 lignes/8 000 caractères, secrets évidents masqués et troncature UTF-8 sûre ; export local de compteurs et d'erreur rédigée livré ; collecte native détaillée reste à qualifier |
...
- **Validation** : suite Rust à 72 tests sur le backend, suite Node à 448 tests avec secrets synthétiques, compteurs invalides et export JSON ; TypeScript/Vite verts.
