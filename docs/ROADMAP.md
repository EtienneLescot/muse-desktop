...
- **Chaîne livrée :** les PRs #18 à #84 ont été fusionnées dans l'ordre de leurs branches de base. La branche finale contient les parcours M0 à M4 déjà câblés dans le client, ainsi que les correctifs de reprise, transcript, terminal Muse, observations Git et l'injection MCP opt-in des sessions locales/distantes.
...
- **Preuves reproductibles :** `npm test` — **587 tests passés** ; `npm run build` — **vert** ; `cargo test --manifest-path src-tauri/Cargo.toml` — **129 tests passés**. Les builds Windows NSIS/MSI et le smoke natif restent des preuves séparées, documentées dans les livraisons M0/M4.
...
- **Prochaine reprise :** fermer les preuves natives M0 (deux workspaces, reprise après autorisation, arrêt/reconnexion et premier lancement Windows), puis traiter le cycle de vie package/source des extensions, la qualification native MCP, le scheduler/les notifications, le navigateur vers le moteur et la distribution signée. Aucun de ces écarts ne doit être présenté comme une parité Codex acquise tant que le scénario réel n'est pas validé.
...
- **Validation :** 3 tests Node couvrent le parsing borné, les doublons et les selectors namespacés ; tests Rust couvrent la validation des parts `skill` et la suite reste verte (582 tests Node, 127 tests Rust).
...
| M3-03 | Installer/désactiver une extension réellement utilisable | Adapté | Présente | Partielle | Intégration | Enregistrement post-probe, runtime persistant explicite, hot-list, hot-reload, rollback d'une révision du registre et injection opt-in de configuration dans le moteur livrés ; restent package/update/source, catalogue d'outils côté host et autorité de permission native |
