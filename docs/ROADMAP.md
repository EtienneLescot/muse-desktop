...
- ◐ **M0-04 — Arrêter et reprendre avec des états fiables**
  ...
  - **Reste (Windows) :** uniquement la phase **arrêt pendant un outil** — bloquée par le défaut d'environnement [`m1-06-run-in-muse-sandbox.md`](evidence/2026-09-27-qualif-native/m1-06-run-in-muse-sandbox.md) (outil shell indisponible en contexte app). **Phases fermées le 27/09/2026** ([`m0-04-stop-terminal.md`](evidence/2026-09-27-qualif-native/m0-04-stop-terminal.md), suite) : **avant le premier token** (arrêt à +1,5 s pré-sortie, `turnId` transmis, résolution 1,0 s, relance immédiate), **réponse tardive** (arrêt à +45 s sur tour vivant, idem), **après la fin** (plus de bouton Stop, UI au repos, tour suivant immédiat). La distinction « demande acceptée / terminal confirmé » est prouvée avec son libellé exact (`Stopping Muse…` capturé en direct, résolution en 1,0 s). **Défaut de libellé restant :** le bouton Stop du tour porte le titre `Stop the running sidecar`.
...
- ◐ **M1-05 — Ouvrir et utiliser un terminal du projet** *(Global : —)*
  ...
  - **Reste :** sortie fonctionnelle, puis validation interactive (ANSI, raccourcis) sur les trois OS. `portable-pty` fournit un shell natif Windows **et** Unix — c'est toujours le ticket le plus proche d'une parité réelle, mais bloqué par ce défaut.
