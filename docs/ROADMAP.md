...
- **Preuves reproductibles :** `npm test -- --run` — **578 tests passés** ; `npm run build` — **vert** ; `cargo test --manifest-path src-tauri/Cargo.toml` — **126 tests passés**. Les builds Windows NSIS/MSI et le smoke natif restent des preuves séparées, documentées dans les livraisons M0/M4.
