...
- ◐ **M0-10 — Réussir le premier lancement**
  ...
  - **Reste (Windows) :** détection sur machine propre, distributions WSL non par défaut, parcours d'authentification réel.
...
- ◐ **M1-05 — Ouvrir et utiliser un terminal du projet** *(Global : —)*
  ...
  - **Reste :** validation native interactive sur les trois OS. `portable-pty` fournit un shell natif Windows **et** Unix — c'est le ticket le plus proche d'une parité réelle.
...
- ◐ **M3-08 — Examiner les résultats des runs** *(code partagé)*
  ...
  - **Reste :** résumé métier sémantique et fin de run native — dépend du host.
...
- ◐ **M4-04 — Faire piloter une application desktop** *(le ticket le plus asymétrique)*
  ...
  - **Reste :** structure riche et relations de documents complexes, qualification d'un selector `computer.*` réellement servi, dialogue de permission Windows, runtimes macOS/Linux. Cette tranche **ne doit pas** être présentée comme un contrôle autonome du bureau.
