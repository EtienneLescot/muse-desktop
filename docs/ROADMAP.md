...
| M0-09 | Conserver les données sans échec silencieux | Adapté | Partielle | Locale | Unitaire | Façade défensive câblée sur tous les modules de persistance connus (`persist`, sessions, automations, notifications, index, mémoire, artifacts, imports, partage, skills, navigateur, composer, thème) ; signalement corruption/indisponibilité/quota et export de récupération ; migration de schémas historiques et restauration guidée restent à qualifier |
...
- **M0-09d — limite restante** : les migrations de versions antérieures et la restauration guidée restent à éprouver avant de changer le format des clés. Une migration future devra copier, valider puis basculer chaque clé avec reprise après interruption.
