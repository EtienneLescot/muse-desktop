...
- **Validation** : `npm test` (560 tests Node, dont cinq tests fixture), build frontend et 110 tests Rust du superviseur verts. La fixture s'exécute aussi dans le job Node de la CI depuis un clone propre.
...
- **M0-02d — corrigé côté client** : après une décision d'autorisation, la chaîne de polling est relancée immédiatement et les nouveaux items ne restent plus bloqués sur l'indicateur réflexif. Une résolution reçue après reconnexion restaure aussi le pont `Muse is resuming` et son placeholder sans exiger que la promesse du clic soit encore en mémoire. Les approbations composées conservent la carte quand le host renvoie `terminal:false`, appliquent le nouveau `currentRequirementId` et remplacent les choix sur `approval/updated`. Les items de raisonnement sont séparés du texte de réponse et peuvent être dépliés ; il reste à valider ce flux avec un moteur live qui émet un item `reasoning`.
