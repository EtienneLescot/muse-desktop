...
- **M0-14g — limite restante** : l'appel `invoke` depuis une webview empaquetée, le pump stdout relié à une vraie fixture enfant et le scénario natif A/B complet avec approbations restent à ajouter avant de déclarer M0-14 complet.
- **Validation** : `npm test` (551 tests Node, dont cinq tests fixture), build frontend et 105 tests Rust du superviseur verts. La fixture s'exécute aussi dans le job Node de la CI depuis un clone propre.
