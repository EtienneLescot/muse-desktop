...
| M0-02 | Reprendre une conversation après fermeture ou panne du moteur | Adapté | Présente | Partielle | UI | Reconnexion explicite câblée via session/read + session/resume ; état de connexion par conversation (`disconnected / connecting / connected / error`) ; réhydratation des items pliés côté client par `itemId` avec conservation des notes locales ; liveness du host visible dans le stream avec attente d'action et état stale ; tests moteur sans tour et UI avec IPC simulé réussis. Reste : validation native des demandes en attente et reprise d'un tour |
...
| M0-07 | Protéger les diagnostics et éviter un crash sur Unicode | Adapté | Présente | Partielle | Unitaire | Aucun wire log brut par défaut ; stderr borné à 20 lignes/8 000 caractères, secrets évidents masqués, troncature UTF-8 sûre et collecte native de compteurs via `collect_diagnostics` ; export local de compteurs et d'erreur rédigée livré ; qualification d'erreurs moteur détaillées reste à faire |
...
| M0-09 | Conserver les données sans échec silencieux | Adapté | Présente | Locale | Unitaire | Façade défensive câblée sur tous les modules de persistance connus ; signalement corruption/indisponibilité/quota, export/import de récupération et migration explicite des alias historiques sans écrasement livrés ; restauration sélective native reste à qualifier |
...
- **Validation** : suite Rust à 73 tests sur le backend, suite Node à 450 tests avec secrets synthétiques, compteurs invalides, résumé multi-worktree et export JSON ; TypeScript/Vite verts.
...
- **M0-09d — restauration livrée** : Settings permet d'importer un export JSON local. Les clés hors namespace et les formats inconnus sont refusés ; les clés existantes sont conservées au premier passage, puis remplacées seulement après confirmation explicite. Le rechargement réhydrate les sessions via le chemin de boot normal.
...
- **M0-09f — limite restante** : une restauration sélective clé par clé et la qualification des formats externes restent à éprouver avant de changer le format des clés. Une future migration devra copier, valider puis basculer chaque clé avec reprise après interruption.
...
- **M0-11d — limite restante** : les messages générés par le moteur restent affichés tels quels ; la checklist complète des erreurs applicatives et titres de chaque surface reste à passer.
...
| M1-05 | Ouvrir et utiliser un terminal du projet | Adapté | Présente | Câblée | Unitaire | Socle PTY persistant livré : shell lié au cwd de la conversation, entrée/sortie bornées, resize, fermeture contrôlée et rendu ANSI SGR courant. Reste : validation native Windows/macOS/Linux, ANSI avancé et raccourcis interactifs |
...
| M1-07 | Consulter les vrais fichiers du projet | Adapté | Présente | Câblée | Intégration | Listing paresseux, lecture bornée et rafraîchissement périodique du snapshot livrés ; restent ouverture native, watcher natif/état obsolète et qualification E2E sur gros dépôts |
...
| M1-13 | Lire une longue conversation confortablement | Adapté | Présente | Partielle | UI | Containment de rendu, scroll instantané et restauration de position par conversation livrés pour les longues listes ; reste à mesurer une session native de 2 000 entrées, mémoire/latence et décider d'une fenêtre virtuelle réelle si nécessaire |
...
- **Rendu :** les séquences ANSI SGR usuelles (couleurs 16/256, gras, atténué, souligné) sont converties en spans stylés sûrs ; les séquences de curseur/titre inconnues sont retirées de la vue sans être interprétées comme HTML.
...
| M3-02 | Connecter un serveur MCP distant | Adapté | Présente | Partielle | Unitaire | Transport streamable HTTP/SSE, bearer token en mémoire, session et bouton de reconnexion explicite livrés ; restent OAuth/secret-store natif, refresh automatique après expiration et qualification réseau multiplateforme |
