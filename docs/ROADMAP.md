...
| M0-02 | Reprendre une conversation après fermeture ou panne du moteur | Adapté | Présente | Partielle | UI | Reconnexion explicite câblée via session/read + session/resume ; réhydratation des items pliés côté client par `itemId` avec conservation des notes locales ; tests moteur sans tour et UI avec IPC simulé réussis. Reste : validation native des demandes en attente et reprise d'un tour |
...
| M0-09 | Conserver les données sans échec silencieux | Adapté | Présente | Locale | Unitaire | Façade défensive câblée sur tous les modules de persistance connus ; signalement corruption/indisponibilité/quota, export et import de récupération bornés avec conservation des clés existantes par défaut ; migrations de schémas historiques et restauration native guidée restent à qualifier |
...
- **M0-09e — limite restante** : les migrations de versions antérieures et une restauration sélective clé par clé restent à éprouver avant de changer le format des clés. Une migration future devra copier, valider puis basculer chaque clé avec reprise après interruption.
...
- **Limites :** les variables d'environnement autorisées, la création atomique session → worktree et la qualification native restent à concevoir. Le setup reste une action explicite et locale.
...
| M2-04 | Préparer l'environnement du worktree | Adapté | Présente | Partielle | Intégration | Commande explicite, profils persistants par workspace, annulation native ciblée, états et sortie bornée livrés ; readiness locale par manifests/outils livrée ; restent variables autorisées, création atomique et qualification native |
