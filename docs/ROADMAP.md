...
- **M0-11d — copie d'erreur livrée** : les surfaces globales, fichiers, projets, paramètres, demandes d'entrée, sélecteurs de dossier, composer et revue Git transforment les préfixes protocole en messages anglais calmes, bornés et masqués (`userFacingError`). Le détail technique reste disponible après un tiret pour faciliter le support ; les messages utilisateur et moteur ne sont pas traduits.
- **M0-11e — limite restante** : la checklist des titres et erreurs spécifiques aux panneaux secondaires doit encore être passée sur chaque surface native.
